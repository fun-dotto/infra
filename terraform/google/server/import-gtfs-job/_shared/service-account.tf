...
resource "google_project_iam_member" "import_gtfs_job" {
  ...
  role    = each.value
  member  = "serviceAccount:${google_service_account.import_gtfs_job.email}"
}

resource "google_secret_manager_secret_iam_member" "import_gtfs_job_gtfs_schedule_url" {
  secret_id = var.gtfs_schedule_url_secret_id
...
resource "google_cloud_run_v2_job_iam_member" "import_gtfs_job_invoker" {
  name   = google_cloud_run_v2_job.import_gtfs_job.name
  role   = "roles/run.invoker"
  member = "serviceAccount:${google_service_account.import_gtfs_job_scheduler.email}"
}
