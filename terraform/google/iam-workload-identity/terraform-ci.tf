# dotto-infra リポジトリの GitHub Actions が terraform-ci SA を借用できるようにする。
# subject は attribute_mapping (google.subject = assertion.repository) により
# "fun-dotto/dotto-infra" となる。
resource "google_service_account_iam_member" "terraform_ci_workload_identity_user" {
  ...
  member             = "principal://iam.googleapis.com/${google_iam_workload_identity_pool.dotto.name}/subject/fun-dotto/dotto-infra"
}
